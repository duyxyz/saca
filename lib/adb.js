...
function getAdbPath() {
  ...
  // Fallback 1: adb.exe next to the compiled executable (process.execPath)
  const exeAdb = join(exeDir, 'adb.exe');
  if (existsSync(exeAdb)) return exeAdb;

  // Fallback 2: adb.exe in parent directory of the executable (for testing inside dist/ folder)
  const parentAdb = join(exeDir, '..', 'adb.exe');
  if (existsSync(parentAdb)) return parentAdb;

  // Fallback 3: adb.exe bundled in project root (for development)
  const localAdb = join(__dirname, '..', 'adb.exe');
  if (existsSync(localAdb)) return localAdb;

  // Fallback 4: adb.exe in current working directory (process.cwd())
  const cwdAdb = join(process.cwd(), 'adb.exe');
  if (existsSync(cwdAdb)) return cwdAdb;
...
/**
 * Terminate the background ADB server.
 */
export function killAdbServer() {
  try {
    const adbPath = getAdbPath();
    spawnSync(adbPath, ['kill-server'], { windowsHide: true });
  } catch (e) {
    // Ignore error
  }
}
